...
final class DashboardRepository implements DashboardRepositoryInterface
{
    ...
    {
        $child = DB::table('daily_report_defects')->join('daily_reports', 'daily_reports.id', '=', 'daily_report_defects.daily_report_id')->join('defects', 'defects.id', '=', 'daily_report_defects.defect_id')->whereBetween('daily_reports.production_date', [$filters->from()->toDateString(), $filters->to()->toDateString()])->whereNotIn('daily_reports.status', ['cancelled'])->when($filters->plantId, fn ($q, $v) => $q->where('daily_reports.plant_id', $v))->when($filters->lineId, fn ($q, $v) => $q->where('daily_reports.line_id', $v))->when($filters->machineId, fn ($q, $v) => $q->where('daily_reports.machine_id', $v))->when($filters->shiftId, fn ($q, $v) => $q->where('daily_reports.shift_id', $v))->when($filters->productId, fn ($q, $v) => $q->where('daily_reports.product_id', $v))->when($filters->checkerId, fn ($q, $v) => $q->where('daily_reports.checker_id', $v))->selectRaw('MIN(daily_reports.id) AS report_id, defects.id, defects.code, defects.name, defects.category, SUM(daily_report_defects.quantity) AS quantity')->groupBy('defects.id', 'defects.code', 'defects.name', 'defects.category')->orderByDesc('quantity')->limit(10)->get();
    ...
    private function metricRow(string $bucket, int $production, int $defects, int $reportId): array { return ['date' => $bucket, 'report_id' => $reportId, 'production_pcs' => $production, 'defect_qty' => $defects, 'defect_rate' => $production > 0 ? round(($defects / $production) * 100, 2) : 0, 'yield' => $production > 0 ? round((($production - $defects) / $production) * 100, 2) : 100]; }
    private function defectExpression(): string { return '(CASE WHEN EXISTS (SELECT 1 FROM daily_report_defects drd WHERE drd.daily_report_id = daily_reports.id) THEN (SELECT COALESCE(SUM(drd2.quantity), 0) FROM daily_report_defects drd2 WHERE drd2.daily_report_id = daily_reports.id) ELSE COALESCE(daily_reports.quantity_defect, 0) END)'; }
    private function scopedQuery(DashboardFilters $filters): Builder { return DailyReport::query()->whereBetween('daily_reports.production_date', [$filters->from()->startOfDay(), $filters->to()->endOfDay()])->whereNotIn('daily_reports.status', ['cancelled'])->when($filters->plantId, fn (Builder $q, int $v) => $q->where('daily_reports.plant_id', $v))->when($filters->lineId, fn (Builder $q, int $v) => $q->where('daily_reports.line_id', $v))->when($filters->shiftId, fn (Builder $q, int $v) => $q->where('daily_reports.shift_id', $v))->when($filters->machineId, fn (Builder $q, int $v) => $q->where('daily_reports.machine_id', $v))->when($filters->productId, fn (Builder $q, int $v) => $q->where('daily_reports.product_id', $v))->when($filters->checkerId, fn (Builder $q, int $v) => $q->where('daily_reports.checker_id', $v)); }
}
