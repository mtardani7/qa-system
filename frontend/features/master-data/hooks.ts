...
export function useMasterProductImport() { const client = useQueryClient(); return useMutation({ mutationFn: (file: File) => importMasterProducts(file), onSuccess: () => client.invalidateQueries({ queryKey: masterKeys.all("products") }) }); }
