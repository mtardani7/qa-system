...
const fieldClass = "mt-1.5 h-10 w-full rounded-lg border border-slate-200 bg-transparent px-3 text-sm text-slate-900 outline-none focus:border-emerald-500 dark:border-slate-700 dark:text-slate-100";
const today = () => new Date().toISOString().slice(0, 10);
const defaults = { output_box: 0, product_type: "FG" as const, defects: [{ defect_id: undefined, quantity: undefined, remarks: "" }], checker_id: undefined, checker_2_id: undefined, production_date: today(), remarks: "", result: undefined, finding_range_box: "" };
...
export function DailyReportFormDialog({ open, report, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    } : { ...defaults, plant_id: plants.length === 1 ? plants[0].id : undefined });
  }, [open, report, reset, lookups, user, plants]);

  if (!open) return null;
  const error = (name: keyof DailyReportFormInput) => errors[name]?.message?.toString();
  const submit = (values: DailyReportFormValues) => {
    const payload = { ...values, defects: values.defects.filter((defect): defect is typeof defect & { defect_id: number; quantity: number } => defect.defect_id !== undefined && defect.quantity !== undefined) };
    ...
    <div className="mx-auto my-6 w-full max-w-6xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
      ...
      {loadingLookups ? <div className="mt-6 space-y-4" aria-label="Loading master data"><Skeleton className="h-10 w-full" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Skeleton className="h-10" /><Skeleton className="h-10" /><Skeleton className="h-10" /><Skeleton className="h-10" /></div><Skeleton className="h-32 w-full" /></div> : <form className="mt-6 space-y-6" onSubmit={handleSubmit(submit)}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Controller name="plant_id" control={control} render={({ field }) => <SearchableSelect label="Plant" value={Number(field.value) || undefined} options={plants} placeholder="Select plant" onChange={field.onChange} error={error("plant_id")} disabled={!isSuperAdmin && plants.length === 1} />} />
        ...
        <div className="grid gap-4 sm:grid-cols-3"><label className="text-sm font-medium">PO Number<input {...register("po_number")} className={fieldClass} />{error("po_number") && <span className="text-xs text-destructive">{error("po_number")}</span>}</label><label className="text-sm font-medium">Output Box<input type="number" {...register("output_box")} className={fieldClass} min="0" />{error("output_box") && <span className="text-xs text-destructive">{error("output_box")}</span>}</label><div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-900 dark:bg-emerald-950/30"><p className="text-xs font-medium uppercase tracking-wide text-emerald-700">Output PCS</p><p className="mt-1 text-2xl font-semibold text-emerald-800 dark:text-emerald-300">{(outputBox * (product?.qty_per_box ?? 0)).toLocaleString()}</p></div></div>
        <div className="grid gap-4"><label className="text-sm font-medium">Findings Range (Box)<input {...register("finding_range_box")} className={fieldClass} placeholder="Example: 1-5" /></label></div>
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <label className="text-sm font-medium">Result<select {...register("result")} className={fieldClass}><option value="">Select result</option><option value="OK">OK</option><option value="OK, WITH NOTED">OK, WITH NOTED</option><option value="SORTIR">SORTIR</option><option value="REJECT">REJECT</option></select></label>
