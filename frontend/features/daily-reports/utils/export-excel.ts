...
export function exportDailyReports(reports: DailyReport[]) {
  const headers = ["Production Date", "Shift", "Machine", "Product Type", "MM Number", "Item Name", "PO Number", "Qty / Box", "Output PCS", "Output Box", "Findings Range (Box)", "Findings Quantity (PCS)", "Finding During Checking", "Defect Description", "Defect Category", "Result / Status", "Checked by QA 1", "Checked by QA 2", "Remarks"];
  const rows = reports.map((report) => [report.production_date, report.shift.name, report.machine.name, report.product_type, report.mm_number, report.product.name || report.product.mm_number, report.po_number, report.qty_per_box, report.output_pcs, report.output_box, report.finding_range_box, report.total_defect, report.finding_observation, report.defects.map((item) => item.remarks).filter(Boolean).join(", "), report.defects.map((item) => item.defect.category).filter(Boolean).join(", "), report.result ?? report.status, report.qa_checker?.name, report.qa_checker_2?.name, report.remarks]);
