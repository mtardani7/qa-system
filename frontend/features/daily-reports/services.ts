...
export async function importDailyReports(file: File, plantId: number) { const form = new FormData(); form.append("file", file); form.append("plant_id", String(plantId)); const { data } = await apiClient.post<ApiResponse<DailyReportImportStatus>>("/daily-reports/import", form, { headers: { "Content-Type": "multipart/form-data" } }); return data.data; }
...
export async function workflowDailyReport(id: number, action: "submit" | "review" | "approve" | "reject" | "lock") { const { data } = await apiClient.post<ApiResponse<DailyReport>>(`/daily-reports/${id}/${action}`); return data.data; }
